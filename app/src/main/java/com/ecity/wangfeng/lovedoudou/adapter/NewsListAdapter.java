...
import com.ecity.wangfeng.lovedoudou.util.DimenUtil;
...
/**
 * 新闻列表适配器 Created by Yoosir on 2016/10/19 0019.
 */
public class NewsListAdapter extends BaseRecyclerViewAdapter<NewsSummary> {
	...
	private void updatePhotoSetViews(PhotoSetViewHolder holder, int position) {
		...
		LinearLayout.LayoutParams groupLayoutParams = (LinearLayout.LayoutParams) holder.newsPictureGroup.getLayoutParams();
		...
		holder.newsPictureGroup.setLayoutParams(groupLayoutParams);
	...
	private void showAndSetPhoto(PhotoSetViewHolder holder, String imgPath1, String imgPath2, String imgPath3) {
		if (imgPath1 != null) {
			loadImage(holder.newsPicture1Iv, imgPath1);
		} else {
			holder.newsPicture1Iv.setVisibility(View.GONE);
		}

		if (imgPath2 != null) {
			loadImage(holder.newsPicture2Iv, imgPath2);
		} else {
			holder.newsPicture2Iv.setVisibility(View.GONE);
		}

		if (imgPath3 != null) {
			loadImage(holder.newsPicture3Iv, imgPath3);
		} else {
			holder.newsPicture3Iv.setVisibility(View.GONE);
		}
	}

	private void loadImage(ImageView view, String path) {
		view.setVisibility(View.VISIBLE);
		Glide.with(MyApplication.getInstance()).load(path).placeholder(R.color.image_place_holder).error(R.mipmap.ic_load_fail).into(view);
	}
	...
	static class PhotoSetViewHolder extends RecyclerView.ViewHolder {
		...
		@Bind(R.id.news_title_tv)
		TextView     newsTitleTv;
		@Bind(R.id.news_picture_1_iv)
		ImageView    newsPicture1Iv;
		@Bind(R.id.news_picture_2_iv)
		ImageView    newsPicture2Iv;
		@Bind(R.id.news_picture_3_iv)
		ImageView    newsPicture3Iv;
		@Bind(R.id.news_picture_group)
		LinearLayout newsPictureGroup;
		@Bind(R.id.news_ptime_tv)
		TextView     newsPtimeTv;
		@Bind(R.id.news_source_tv)
		TextView     newsSourceTv;
